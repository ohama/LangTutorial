...
3. **Add context at each inference site:**
   For each expression type, push appropriate InferContext before recursing:
   ...
   - **List literal:**
     ```fsharp
     | List (exprs, span) ->
         // Add InListElement context for each element
     ```
...
```
  ...
  <done>
Infer.fs has inferWithContext that threads context stack through inference, uses unifyWithContext for unification calls, and throws TypeException for unbound variables.
  </done>
</task>
...
```
  ...
  <done>
TypeCheck.fs catches TypeException, converts to Diagnostic, provides both string-based (backward compat) and Diagnostic-based error results.
  </done>
</task>
...
- Type errors: Now include richer messages
  ...
  <verify>
```bash
dotnet run --project FunLang.Tests 2>&1 | tail -3
make -C tests 2>&1 | tail -3
echo "let x = 1 + 2 in x * 3" | dotnet run --project FunLang
```
All tests pass, REPL works, type error messages are richer than before.
  </verify>
  <done>
...
- inferWithContext maintains context stack at each inference site

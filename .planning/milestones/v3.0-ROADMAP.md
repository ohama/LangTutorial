...
복합 데이터 타입, 패턴 매칭, 표준 라이브러리를 추가하여 FunLang을 실용적인 함수형 언어로 발전
...
- Self-hosted Prelude — FunLang으로 표준 라이브러리 작성 (dogfooding)
- evalToEnv pattern — 중첩 let 바인딩을 환경으로 수집
- First-match semantics — 패턴 매칭에서 첫 번째 매치 사용
- Right-associative cons — `1 :: 2 :: []` = `1 :: (2 :: [])`

**Issues Resolved:**

- compose + map 조합 시 무한 루프 (문서화, 향후 수정 예정)

**Technical Debt:**

- 완전성 검사는 경고만 출력 (에러로 변경 고려)
